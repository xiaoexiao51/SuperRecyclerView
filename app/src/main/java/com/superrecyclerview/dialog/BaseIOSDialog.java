...
//        params.height = (int) (d.heightPixels * 0.6); // 高度设置为屏幕的0.6
    ...
    private void setSheetItems() {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if (mItems.size() == 1) {
                if (showTitle) {
                    textView.setBackgroundResource(R.drawable.shap_dialog_ios_bottom);
                } else {
                    textView.setBackgroundResource(R.drawable.shap_dialog_ios_single);
                }
            } else {
                if (showTitle) {
                    if (i >= 0 && i < mItems.size() - 1) {
                        textView.setBackgroundResource(R.drawable.shap_dialog_ios_middle);
                    } else {
                        textView.setBackgroundResource(R.drawable.shap_dialog_ios_bottom);
                    }
                } else {
                    if (i == 0) {
                        textView.setBackgroundResource(R.drawable.shap_dialog_ios_top);
