...
public class GlideUtils {
    ...
    public static void loadWithDefult(Context mContext, String path, ImageView mImageView) {
        ...
        if (CommonUtils.checkNetwork(mContext) && !CommonUtils.isWifi(mContext) && !download_img) {
            mImageView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_bg));
            return;
        }
        Glide.with(mContext).load(path).placeholder(R.color.color_bg).error(R.color.color_bg).centerCrop().into(mImageView);
    }
    ...
    public static void loadWithBlack(Context mContext, String path, ImageView mImageView) {
        boolean download_img = SPUtils.getBoolean(mContext, "download_img", false);
        if (CommonUtils.checkNetwork(mContext) && !CommonUtils.isWifi(mContext) && !download_img) {
            mImageView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_blue));
            return;
        }
        Glide.with(mContext).load(path).placeholder(R.color.color_blue).error(R.color.color_blue).fitCenter().into(mImageView);
    }
